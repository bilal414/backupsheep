...
IBM_OBJECT_METADATA_KEY = "ibm_cos_s3_object"


def _s3_client(ibm, encryption_key):
    return bounded_boto3_client(
        "s3",
        allow_retries=True,
        aws_access_key_id=bs_decrypt(ibm.access_key, encryption_key),
        aws_secret_access_key=bs_decrypt(ibm.secret_key, encryption_key),
        region_name=ibm.region.code,
        endpoint_url=f"https://{ibm.endpoint}",
        config=Config(
...
def storage_ibm(stored_backup):
    try:
        ...
        ibm = storage.storage_ibm
        prefix = ibm.prefix or ""
        ...
        upload_verified_s3(
            stored_backup,
            client=_s3_client(ibm, storage.account.get_encryption_key()),
            bucket=ibm.bucket_name,
            key=key,
            local_path=f"_storage/{artifact_identity.filename}",
            metadata_key=IBM_OBJECT_METADATA_KEY,
        ...
        raise _safe_upload_exception(
            StorageIBMUploadFailedError, stored_backup, error
        ) from error
import boto3
