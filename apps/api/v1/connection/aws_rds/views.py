...
class CoreAWSRDSView(ReadWriteSerializerMixin, viewsets.ModelViewSet):
    permission_classes = (IsAuthenticated, CoreAWSRDSViewPermissions,)
    read_serializer_class = CoreAWSRDSConnectionReadSerializer
    write_serializer_class = CoreAWSRDSConnectionWriteSerializer
    ...
    filterset_class = CoreAWSRDSFilter
    ...
    def endpoints(self, request):
        endpoints = CoreConnectionLocation.objects.filter(integrations__code="aws_rds").values()
    ...
    def objects(self, request, pk=None):
        try:
            connection = self.get_object()
            eligible_objects = connection.auth_aws_rds.get_eligible_objects(object_type=self.request.query_params.get("object_type"))
            for eligible_object in eligible_objects:
                query = Q(unique_id=eligible_object["DBInstanceIdentifier"], node__connection=connection)
                query &= ~Q(node__status=CoreNode.Status.DELETE_REQUESTED)
                if CoreAWSRDS.objects.filter(query).exists():
