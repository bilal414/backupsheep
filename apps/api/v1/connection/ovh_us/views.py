...
class CoreOVHUSView(ReadWriteSerializerMixin, viewsets.ModelViewSet):
    ...
    filterset_class = CoreOVHUSFilter
    ...
    def endpoints(self, request):
        endpoints = CoreConnectionLocation.objects.filter(integrations__code="ovh_us").values()
    ...
    def objects(self, request, pk=None):
        try:
            connection = self.get_object()
            eligible_objects = connection.auth_ovh_us.get_eligible_objects(object_type=self.request.query_params.get("object_type"))
            for eligible_object in eligible_objects:
                ...
                if CoreOVHUS.objects.filter(query).exists():
