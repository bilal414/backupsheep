...
class CoreVultrView(ReadWriteSerializerMixin, viewsets.ModelViewSet):
    permission_classes = (IsAuthenticated, CoreVultrViewPermissions,)
    read_serializer_class = CoreVultrConnectionReadSerializer
    write_serializer_class = CoreVultrConnectionWriteSerializer
    ...
    filterset_class = CoreVultrFilter
    ...
    def endpoints(self, request):
        endpoints = CoreConnectionLocation.objects.filter(integrations__code="vultr").values()
    ...
    def objects(self, request, pk=None):
        try:
            connection = self.get_object()
            eligible_objects = connection.auth_vultr.get_eligible_objects(object_type=self.request.query_params.get("object_type"))
