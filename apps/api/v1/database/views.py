...
from apps.api.v1.database.filters import CoreDatabaseFilter
from apps.api.v1.database.permissions import CoreDatabaseViewPermissions
from apps.api.v1.database.serializers import (
...
class CoreDatabaseView(ReadWriteSerializerMixin, viewsets.ModelViewSet):
    ...
    def totals(self, request):
        ...
        all_totals = {
            ...
            "storage": 0,
            "in_progress": CoreDatabaseBackup.objects.filter(
                database__in=databases, status__in=UtilBackup.ACTIVE_STATUSES
            ).count(),
        }
