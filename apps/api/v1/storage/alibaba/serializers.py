...
from apps.console.connection.models import CoreAlibabaRegion
from apps.console.storage.models import CoreStorageAliBaba, CoreStorage



class CoreAlibabaRegionSerializer(serializers.ModelSerializer):
    class Meta:
        model = CoreAlibabaRegion
        fields = "__all__"
        datatables_always_serialize = ("id",)


class CoreStorageAliBabaReadSerializer(StorageCredentialReadSerializerMixin, serializers.ModelSerializer):
    credential_fields = ("access_key", "secret_key")
    region = CoreAlibabaRegionSerializer()

    class Meta:
        model = CoreStorageAliBaba
        fields = (
            "id",
            "no_delete",
            "access_key",
...
class CoreStorageAliBabaWriteSerializer(StorageCredentialWriteSerializerMixin, serializers.ModelSerializer):
    credential_fields = ("access_key", "secret_key")
...
class CoreStorageReadSerializer(serializers.ModelSerializer):
    ...
    storage_alibaba = CoreStorageAliBabaReadSerializer()
...
class CoreStorageWriteSerializer(serializers.ModelSerializer):
    ...
    storage_alibaba = CoreStorageAliBabaWriteSerializer()
    type = serializers.HiddenField(
        default=serializers.CreateOnlyDefault(StorageDefault("alibaba"))
    )

    class Meta:
        model = CoreStorage
        ref_name = "Storage Alibaba Write"
        fields = "__all__"

    def create(self, validated_data):
        storage_alibaba = validated_data.pop("storage_alibaba", [])
        instance = CoreStorage.objects.create(**validated_data)
        storage_alibaba["storage"] = instance
        CoreStorageAliBaba.objects.create(**storage_alibaba)
        return instance

    def update(self, instance, validated_data):
        storage_alibaba = validated_data.pop("storage_alibaba", [])
        super().update(instance.storage_alibaba, storage_alibaba)
