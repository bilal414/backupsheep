...
from apps.console.storage.models import CoreStorageCloudflare, CoreStorage


class CoreStorageCloudflareReadSerializer(StorageCredentialReadSerializerMixin, serializers.ModelSerializer):
    credential_fields = ("access_key", "secret_key")

    class Meta:
        model = CoreStorageCloudflare
        fields = (
            "id",
            "no_delete",
            "access_key",
...
class CoreStorageCloudflareWriteSerializer(StorageCredentialWriteSerializerMixin, serializers.ModelSerializer):
    credential_fields = ("access_key", "secret_key")
...
class CoreStorageReadSerializer(serializers.ModelSerializer):
    ...
    storage_cloudflare = CoreStorageCloudflareReadSerializer()
    ...
    class Meta:
        model = CoreStorage
        fields = "__all__"
        ref_name = "Storage Cloudflare Read"
        datatables_always_serialize = (
            "id",
            "name",
        )
...
class CoreStorageWriteSerializer(serializers.ModelSerializer):
    ...
    storage_cloudflare = CoreStorageCloudflareWriteSerializer()
    type = serializers.HiddenField(
        default=serializers.CreateOnlyDefault(StorageDefault("cloudflare"))
    )

    class Meta:
        model = CoreStorage
        ref_name = "Storage Cloudflare Write"
        fields = "__all__"

    def create(self, validated_data):
        storage_cloudflare = validated_data.pop("storage_cloudflare", [])
        instance = CoreStorage.objects.create(**validated_data)
        storage_cloudflare["storage"] = instance
        CoreStorageCloudflare.objects.create(**storage_cloudflare)
        return instance

    def update(self, instance, validated_data):
        storage_cloudflare = validated_data.pop("storage_cloudflare", [])
        super().update(instance.storage_cloudflare, storage_cloudflare)
