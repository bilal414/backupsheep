...
from apps.console.connection.models import CoreIBMRegion
from apps.console.storage.models import CoreStorageIBM, CoreStorage



class CoreIBMRegionSerializer(serializers.ModelSerializer):
    class Meta:
        model = CoreIBMRegion
        fields = "__all__"
...
class CoreStorageIBMReadSerializer(StorageCredentialReadSerializerMixin, serializers.ModelSerializer):
    credential_fields = ("access_key", "secret_key")
    region = CoreIBMRegionSerializer()

    class Meta:
        model = CoreStorageIBM
        fields = (
            "id",
            "no_delete",
            "access_key",
...
class CoreStorageIBMWriteSerializer(StorageCredentialWriteSerializerMixin, serializers.ModelSerializer):
    credential_fields = ("access_key", "secret_key")
...
class CoreStorageReadSerializer(serializers.ModelSerializer):
    ...
    storage_ibm = CoreStorageIBMReadSerializer()
...
class CoreStorageWriteSerializer(serializers.ModelSerializer):
    ...
    storage_ibm = CoreStorageIBMWriteSerializer()
    type = serializers.HiddenField(
        default=serializers.CreateOnlyDefault(StorageDefault("ibm"))
    )

    class Meta:
        model = CoreStorage
        ref_name = "Storage IBM Write"
        fields = "__all__"

    def create(self, validated_data):
        storage_ibm = validated_data.pop("storage_ibm", [])
        instance = CoreStorage.objects.create(**validated_data)
        storage_ibm["storage"] = instance
        CoreStorageIBM.objects.create(**storage_ibm)
        return instance

    def update(self, instance, validated_data):
        storage_ibm = validated_data.pop("storage_ibm", [])
        super().update(instance.storage_ibm, storage_ibm)
