...
from apps.console.connection.models import CoreIonosRegion
from apps.console.storage.models import CoreStorageIonos, CoreStorage



class CoreIonosRegionSerializer(serializers.ModelSerializer):
    class Meta:
        model = CoreIonosRegion
        fields = "__all__"
        datatables_always_serialize = ("id",)


class CoreStorageIonosReadSerializer(StorageCredentialReadSerializerMixin, serializers.ModelSerializer):
    credential_fields = ("access_key", "secret_key")
    region = CoreIonosRegionSerializer()

    class Meta:
        model = CoreStorageIonos
        fields = (
            "id",
            "no_delete",
            "access_key",
...
class CoreStorageIonosWriteSerializer(StorageCredentialWriteSerializerMixin, serializers.ModelSerializer):
    credential_fields = ("access_key", "secret_key")
...
class CoreStorageReadSerializer(serializers.ModelSerializer):
    ...
    storage_ionos = CoreStorageIonosReadSerializer()
...
class CoreStorageWriteSerializer(serializers.ModelSerializer):
    ...
    storage_ionos = CoreStorageIonosWriteSerializer()
    type = serializers.HiddenField(
        default=serializers.CreateOnlyDefault(StorageDefault("ionos"))
    )

    class Meta:
        model = CoreStorage
        ref_name = "Storage Ionos Write"
        fields = "__all__"

    def create(self, validated_data):
        storage_ionos = validated_data.pop("storage_ionos", [])
        instance = CoreStorage.objects.create(**validated_data)
        storage_ionos["storage"] = instance
        CoreStorageIonos.objects.create(**storage_ionos)
        return instance

    def update(self, instance, validated_data):
        storage_ionos = validated_data.pop("storage_ionos", [])
        super().update(instance.storage_ionos, storage_ionos)
