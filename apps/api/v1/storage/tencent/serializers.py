...
from apps.console.connection.models import CoreTencentRegion
from apps.console.storage.models import CoreStorageTencent, CoreStorage



class CoreTencentRegionSerializer(serializers.ModelSerializer):
    class Meta:
        model = CoreTencentRegion
        fields = "__all__"
        datatables_always_serialize = ("id",)


class CoreStorageTencentReadSerializer(StorageCredentialReadSerializerMixin, serializers.ModelSerializer):
    credential_fields = ("access_key", "secret_key")
    region = CoreTencentRegionSerializer()

    class Meta:
        model = CoreStorageTencent
        fields = (
            "id",
            "no_delete",
            "access_key",
...
class CoreStorageTencentWriteSerializer(StorageCredentialWriteSerializerMixin, serializers.ModelSerializer):
    credential_fields = ("access_key", "secret_key")
...
class CoreStorageReadSerializer(serializers.ModelSerializer):
    ...
    storage_tencent = CoreStorageTencentReadSerializer()
...
class CoreStorageWriteSerializer(serializers.ModelSerializer):
    ...
    storage_tencent = CoreStorageTencentWriteSerializer()
    type = serializers.HiddenField(
        default=serializers.CreateOnlyDefault(StorageDefault("tencent"))
    )

    class Meta:
        model = CoreStorage
        ref_name = "Storage Tencent Write"
        fields = "__all__"

    def create(self, validated_data):
        storage_tencent = validated_data.pop("storage_tencent", [])
        instance = CoreStorage.objects.create(**validated_data)
        storage_tencent["storage"] = instance
        CoreStorageTencent.objects.create(**storage_tencent)
        return instance

    def update(self, instance, validated_data):
        storage_tencent = validated_data.pop("storage_tencent", [])
        super().update(instance.storage_tencent, storage_tencent)
