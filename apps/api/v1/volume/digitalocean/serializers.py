...
_DIGITALOCEAN_RESOURCE_MESSAGES = {
    "CONNECTION_PROVIDER_MISMATCH": "Select a DigitalOcean connection for this volume.",
    "RESOURCE_ID_REQUIRED": "Select a DigitalOcean volume to link.",
    "RESOURCE_TYPE_MISMATCH": "The selected DigitalOcean resource is not a volume.",
    "RESOURCE_ALREADY_LINKED": "This DigitalOcean volume is already linked.",
    "PROVIDER_NOT_FOUND": "DigitalOcean could not find the requested volume.",
    "PROVIDER_DUPLICATE_MATCH": "DigitalOcean returned multiple matches for this volume.",
    "PROVIDER_MALFORMED_RESPONSE": "DigitalOcean returned an incomplete or malformed volume response.",
    "PROVIDER_OWNERSHIP_MISMATCH": "The DigitalOcean volume did not match the expected resource type or identity.",
...
def _validate_submitted_metadata(metadata, resource_id):
    ...
    volume.  Fresh provider discovery always replaces this payload before save.
    """
    if metadata is None:
        return
    ...
    for key in ("resource_type", "_bs_resource_type"):
        ...
        if value in (None, ""):
            continue
        if str(value).strip().casefold() != "volume":
            _digitalocean_error("PROVIDER_OWNERSHIP_MISMATCH", field="metadata")


def _discover_digitalocean_volume(connection, resource_id):
    """Rediscover one volume from the provider's authoritative inventory."""
    try:
    ...
    if explicit_types and not explicit_types.issubset({"volume"}):
        raise DigitalOceanAPIError("PROVIDER_OWNERSHIP_MISMATCH")
    ...
    provider["_bs_resource_type"] = "volume"
    return provider


def _digitalocean_volume_duplicate_exists(connection, resource_id):
...
class CoreVolumeDigitalOceanReadSerializer(serializers.ModelSerializer):
...
class CoreVolumeDigitalOceanWriteSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if resource_type not in (None, "", "volume"):
        ...
        try:
            provider = _discover_digitalocean_volume(connection, resource_id)
        except Exception as error:
            _raise_digitalocean_resource_error(error)

        if _digitalocean_volume_duplicate_exists(connection, resource_id):
