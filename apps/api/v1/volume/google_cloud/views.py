...
from apps.api.v1.volume.google_cloud.filters import CoreVolumeGoogleCloudFilter
from apps.api.v1.volume.google_cloud.permissions import CoreVolumeGoogleCloudViewPermissions
from apps.api.v1.volume.google_cloud.serializers import (
    CoreVolumeGoogleCloudReadSerializer,
    CoreVolumeGoogleCloudWriteSerializer,
)
...
class CoreVolumeGoogleCloudView(ReadWriteSerializerMixin, viewsets.ModelViewSet):
    permission_classes = (
        IsAuthenticated,
        CoreVolumeGoogleCloudViewPermissions,
    )
    read_serializer_class = CoreVolumeGoogleCloudReadSerializer
    write_serializer_class = CoreVolumeGoogleCloudWriteSerializer
    ...
    filterset_class = CoreVolumeGoogleCloudFilter
    ...
    def totals(self, request):
        ...
        query &= Q(node__type=CoreNode.Type.VOLUME)
        query &= ~Q(node__status=CoreNode.Status.DELETE_REQUESTED)
        nodes = scope_direct_node_queryset(request, CoreGoogleCloud.objects.filter(query))
        all_totals = {
            "nodes": nodes.count(),
