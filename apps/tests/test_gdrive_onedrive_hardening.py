...
class _DriveClient:
    ...
    def put(self, url, **kwargs):
        ...
        if content_range.startswith("bytes */"):
            item = next((value for value in self.objects.values() if value.get("mimeType") == google_drive.ZIP_MIME), None)
        ...
        item = next(value for value in self.objects.values() if value.get("mimeType") == google_drive.ZIP_MIME)
