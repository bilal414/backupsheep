...
verify_target() {
    ...
    [[ "$(target_query "$container" "SELECT type.typtype || '|' || type.typnotnull || '|' || type.typbasetype::pg_catalog.regtype::text FROM pg_catalog.pg_type type WHERE type.oid='public.fixture_code'::pg_catalog.regtype")" == 'd|t|text' ]] \
