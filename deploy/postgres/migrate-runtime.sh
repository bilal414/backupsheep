...
default_acl="$(psql_source "SELECT owner.rolname || '|' || COALESCE(namespace.nspname, '<global>') || '|' || defaults.defaclobjtype || '|' || CASE WHEN acl.grantee=0 THEN 'PUBLIC' ELSE grantee.rolname END || '|' || grantor.rolname || '|' || acl.privilege_type || '|' || acl.is_grantable FROM pg_default_acl defaults JOIN pg_roles owner ON owner.oid=defaults.defaclrole LEFT JOIN pg_namespace namespace ON namespace.oid=defaults.defaclnamespace CROSS JOIN LATERAL aclexplode(defaults.defaclacl) acl LEFT JOIN pg_roles grantee ON grantee.oid=acl.grantee JOIN pg_roles grantor ON grantor.oid=acl.grantor ORDER BY 1")"
default_acl_records="$(psql_source "SELECT owner.rolname || '|' || COALESCE(namespace.nspname, '<global>') || '|' || defaults.defaclobjtype FROM pg_default_acl defaults JOIN pg_roles owner ON owner.oid=defaults.defaclrole LEFT JOIN pg_namespace namespace ON namespace.oid=defaults.defaclnamespace ORDER BY 1")"
