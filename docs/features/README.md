...
| Operations | A scoped dashboard, source inventory, run history, durable execution status, retry and reconciliation state, activity logs, cancellation, deletion, transfer, and download actions where applicable |
...
- Automatic in-console restore is not universal. The exact matrix is documented
  in [Restores](restores.md); download remains a separate recovery option.
