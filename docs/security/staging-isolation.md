...
| Database source/restore | `10002:10002` | `database_workdir` | backup writer `10998`, upload reader `10999`, database-restore reader `10994` |
| Files source/restore | `10003:10003` | `files_workdir` | backup writer `10998`, upload reader `10999`, files-restore reader `10993` |
| Storage upload | `10004:10004` | `storage_workdir` | upload reader `10999`; restore writer `10995` and target readers `10993`/`10994` |
...
2. It calls `create_ciphertext_fence(backup_uuid)`. The fence is
   `/var/lib/backupsheep/transfer/<canonical-backup-uuid>`, owned by that source
   UID and reader group `10999`, mode `2750`. The transfer root is root-owned by
   writer group `10998`, setgid and sticky (`3771`). Storage has only reader group
   `10999`, so it can traverse a known finished fence but cannot list the transfer
   root or pre-create a UUID entry;
   source lanes cannot remove a fence owned by the other source UID.
...
5. The storage worker calls `private_storage_root()` and `open_ciphertext`, checks it against the durable
...
cross-installation reuse, cross-lane cleanup and partial/unpublished ciphertext all
fail closed.
...
```text
sha256("BackupSheep/staging-layout/v1|<installation-id>|<intent>")
```

Allowed intents are `new-empty-v1` and `migrate-empty-legacy-v1`. The provisioner
first proves that every target is a dedicated mount and all new work/transfer
volumes, including the reverse restore-transfer volume, are empty. For migration,
...
other's plaintext, storage cannot create forward-transfer entries, source roles
cannot create reverse-transfer entries, and each restore lane can read only its own
